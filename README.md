...
- **[Системный промпт для PHP разработки на VSCode](ChatGPT/SLA-prompts/PHP/AGENTS.md)** задаёт “системную роль” и правила работы ассистента как Senior PHP Engineer/Architect со специализацией в VirtueMart 4.6.4 (язык ответа — русский, акцент на типы/тесты/безопасность/проверки). Он нужен, чтобы стандартизировать взаимодействие: какие уточнения задавать, какие принципы соблюдать и в каком формате всегда выдавать результат (бриф → план → код → проверка → альтернативы). Также он фиксирует интеграцию с Codex/IDE/CLI и ограничения (не светить секреты, санитизация, лицензии) и отдельно оговаривает, что писать код можно только по явному запросу.
